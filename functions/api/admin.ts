...
const TABLES: Record<string, TableConfig> = {
  clinics: {
    select: 'id,name,phone,email,address,logo_url,brand_color,active,enable_patient_photos,enable_prescription_medications,created_at',
    order: 'created_at',
    search: ['name', 'phone', 'email', 'address'],
  },
  profiles: {
    select: 'id,clinic_id,name,email,phone,role,active,invite_code,created_at',
    order: 'created_at',
    search: ['name', 'email', 'phone', 'role'],
  },
  patients: {
    select: 'id,clinic_id,patient_code,name,phone,age,dob,gender,email,address,emergency_contact,photo_url,created_at',
    order: 'created_at',
    search: ['patient_code', 'name', 'phone', 'email'],
  },
  appointments: {
    select: 'id,clinic_id,patient_id,doctor_id,appointment_time,status,notes,reminder_status,reminder_sent_at,reminder_status_at,op_fee_amount,op_fee_status,op_fee_waiver_reason,op_fee_waived_by,op_fee_waived_at,created_at',
    order: 'appointment_time',
    search: ['status', 'op_fee_status', 'notes', 'reminder_status'],
  },
  patient_visits: {
  ...
  invoices: {
    select: 'id,clinic_id,patient_id,visit_id,total_amount,paid_amount,due_amount,status,invoice_type,payment_category,notes,created_at',
    order: 'created_at',
    search: ['status', 'invoice_type', 'payment_category', 'notes'],
  },
  payments: {
    select: 'id,clinic_id,patient_id,invoice_id,amount,payment_method,payment_category,collected_by,notes,created_at',
    order: 'created_at',
    search: ['payment_method', 'payment_category', 'notes'],
  },
  files: {
    select: 'id,clinic_id,patient_id,visit_id,file_type,file_name,file_url,file_note,uploaded_by,xray_amount,xray_fee_status,created_at',
    order: 'created_at',
    search: ['file_type', 'file_name', 'file_note', 'xray_fee_status'],
  },
  staff_invites: {
    select: 'id,clinic_id,email,name,role,invite_code,invited_by,accepted_at,created_at',
    order: 'created_at',
...
export async function onRequestGet(context: { request: Request; env: Env }) {
  ...
  if (mode === 'table') {
    ...
    let query: any = admin.from(table).select(config.select).limit(5000).order(config.order, { ascending: false });
  ...
  const [clinics, staff, patients, visits, appointmentsToday, paymentsToday, pendingInvoices, activeClinics] = await Promise.all([
    ...
    admin.from('payments').select('amount').gte('created_at', start).lt('created_at', end).limit(5000),
    admin.from('invoices').select('due_amount').gt('due_amount', 0).limit(5000),
